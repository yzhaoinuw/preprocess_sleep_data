function MeanPowerSpectrumPerGroup(Analyse,Settings,SaveTF)
   ...
   TransitionTime2Remove=Analyse(nAnalyse).TransitionTime2Remove;
    ...
    for nmouse=1:length(Analyse(nAnalyse).Mice)
        ...
        if length(idmousebaseline)==1 && length(idmousemanip)==1
            ...
            %computepower psectrum if do not exist
            PrefTF=[Analyse(nAnalyse).Mice{nmouse} '-' Analyse(nAnalyse).Baseline];
            

            CurrDir=HeaderBaseline.Info.FilesDir;  
            MatFileName=sprintf('%s\\Computed Data\\%s %s process %06.1f min-%06.1f min Pass1.mat',CurrDir,PrefTF,HeaderBaseline.Info.ExpFileName(1:end-4),T0,T0+Duration);
            CurrCh=AllChannel(idmousemanip);
            if exist(MatFileName)
                load(MatFileName,'TF');
            else
                clear ProcessInfo;
                ProcessInfo(1,1:10)={CurrCh 1 1 1 [] [] 0 Settings.ApplyICA Settings.UseWhitening []}; %calcul a realiser
                Pass=1;
                [~,~,~,~,TF,~,~,~,~,~,~,~,~,~,~]=ComputeWUW(PrefTF,T0,Duration,ProcessInfo,Pass,HeaderBaseline.Info,Settings.Artef,Settings.FigSave,...
                            Settings.EpochDuration,0,Settings.ProcessDataPerXMin,Settings.Filt,Settings.MTT,Settings.LightOn,[],Settings.ARmodel);
            end
            
            
            Analyse(nAnalyse).F{nmouse}=TF.Chan(CurrCh,CurrCh).F;
            
            TFBaseline{nmouse}=TF.Chan(CurrCh,CurrCh);
            if SaveTF==1
            ...
            Tok=zeros(1,length(TFBaseline{nmouse}.time));
            Stateok=find(DataLimState.Event.Hypno==StateCode2Process);
            DebTime=DataLimState.Event.Deb(Stateok);
            EndTime=DataLimState.Event.End(Stateok);
            for nlim=1:length(DebTime)
                Tok=Tok | (TFBaseline{nmouse}.time>=DebTime(nlim) & TFBaseline{nmouse}.time<=EndTime(nlim));
            end

            Analyse(nAnalyse).MeanPWBL{nmouse}=mean(TFBaseline{nmouse}.S(Tok,:),1);
            NbLine=size(PwBL,1);
            ...
            for nband=1:length(Settings.Band2Analyse)
                idband=TF.Chan(CurrCh,CurrCh).F>=Settings.Band2Analyse{nband}(1) & TF.Chan(CurrCh,CurrCh).F<=Settings.Band2Analyse{nband}(2) ;
            ...
            DataAllLim{nAnalyse,nmouse}.Baseline=DataLimState;
            %computepower psectrum if do not exist
            PrefTF=[Analyse(nAnalyse).Mice{nmouse} '-' Analyse(nAnalyse).Manip];
            

            CurrDir=HeaderManip.Info.FilesDir;  
            MatFileName=sprintf('%s\\Computed Data\\%s %s process %06.1f min-%06.1f min Pass1.mat',CurrDir,PrefTF,HeaderManip.Info.ExpFileName(1:end-4),T0,T0+Duration);
            CurrCh=AllChannel(idmousemanip);
            if exist(MatFileName)
                load(MatFileName,'TF');
                if isempty(TF) || exist('TF')==0
                    clear ProcessInfo;
                    ProcessInfo(1,1:10)={CurrCh 1 1 1 [] [] 0 Settings.ApplyICA Settings.UseWhitening []}; %calcul a realiser
                    Pass=1;
                    [~,~,~,~,TF,~,~,~,~,~,~,~,~,~,~]=ComputeWUW(PrefTF,T0,Duration,ProcessInfo,Pass,HeaderManip.Info,Settings.Artef,Settings.FigSave,...
                            Settings.EpochDuration,0,Settings.ProcessDataPerXMin,Settings.Filt,Settings.MTT,Settings.LightOn,[],Settings.ARmodel);
            ...
            TFManip{nmouse}=TF.Chan(CurrCh,CurrCh);
            if SaveTF==1
            ...
            Tok=zeros(1,length(TFManip{nmouse}.time));
            Stateok=find(DataLimState.Event.Hypno==StateCode2Process);
            DebTime=DataLimState.Event.Deb(Stateok);
            EndTime=DataLimState.Event.End(Stateok);
            for nlim=1:length(DebTime)
                Tok=Tok | (TFManip{nmouse}.time>=DebTime(nlim) & TFManip{nmouse}.time<=EndTime(nlim));
            end

            Analyse(nAnalyse).MeanPWManip{nmouse}=mean(TFManip{nmouse}.S(Tok,:),1);
